...
*Which of my own code breaks* is answered from the installation's real state, not the core changelog — that question is about your code, and the changelog does not know it. *How do I migrate a specific hit*, once a matching entry is found, can be answered from the changelog itself: it ships migration guidance offline, so that step should not fall back to training-data guessing. Each tool defaults to a compact, token-efficient summary:
